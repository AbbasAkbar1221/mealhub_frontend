...
// const DishCard = () => {
//   const [loading, setLoading] = useState(false); // Loading state for the entire component
//   const [disabledDishes, setDisabledDishes] = useState([]); // Track dishes that are being processed
//   const [error, setError] = useState(null);
//   const { counterId } = useParams();
//   const dispatch = useDispatch();
//   const counterDetails = useSelector((state) => state.counter.details);
...
//   const handleAddToCart = (dishId) => {
//     if (disabledDishes.includes(dishId) || loading) return; // Prevent multiple clicks on the same dish

//     setDisabledDishes((prev) => [...prev, dishId]); // Disable the button for this dish
...
//         setDisabledDishes((prev) => prev.filter((id) => id !== dishId)); // Re-enable the button for the processed dish
//       }
//     };
...
//                 (disabledDishes.includes(dish._id)) ? "opacity-50 cursor-not-allowed" : ""
//               }`}
//               onClick={() => handleAddToCart(dish._id)}
//               disabled={disabledDishes.includes(dish._id)} // Disable the button while loading or if already clicked
//             >
//               {disabledDishes.includes(dish._id) ? "Adding..." : "Add to Cart"}
//             </button>
//           </li>
//         ))}
//       </ul>
